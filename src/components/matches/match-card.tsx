    ...
    <Link href={`/dashboard/matches/${match.matchId}`} className="card card-interactive block p-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          ...
          <span className="text-sm font-medium text-white truncate">{getCountryName(match.homeTeam)}</span>
        </div>
        <div className="px-3 text-center shrink-0">
          {match.status === "scheduled" ? (
            <span className="text-sm text-muted">vs</span>
          ) : (
            <span className="text-base font-bold text-white">{match.homeScore ?? "-"} - {match.awayScore ?? "-"}</span>
          )}
        </div>
        <div className="flex items-center gap-2.5 flex-1 min-w-0 justify-end">
          <span className="text-sm font-medium text-white truncate text-right">{getCountryName(match.awayTeam)}</span>
